...
def manual_ar_process(ar_coeffs, noise_var, n):
    ...
    for t in range(p, n):
        ar_series[t] = (
            sum(ar_coeffs[i] * ar_series[t - i - 1] for i in range(p)) + noise[t]
        )
...
def manual_arma_process(ar_coeffs, ma_coeffs, noise_var, n):
    ...
    for t in range(p, n):
        arma_series[t] = (
            sum(ar_coeffs[i] * arma_series[t - i - 1] for i in range(p))
            + noise[t]
            + sum(ma_coeffs[i] * noise[t - i - 1] for i in range(q))
        )
...
def yule_walker(data, order):
    ...
    autocovariances = [
        np.cov(data[:-k], data[k:])[0, 1] if k > 0 else np.var(data, ddof=0)
        for k in range(order + 1)
    ]

    # Create the autocovariance matrix R
    R = np.array(
        [[autocovariances[abs(i - j)] for j in range(order)] for i in range(order)]
    )
...
def plot_roots(unit_roots, padding=1):
    ...
    circle = plt.Circle(
        (0, 0), 1, color="blue", fill=False, linewidth=0.8, linestyle="--"
    )
    ax = plt.gca()
